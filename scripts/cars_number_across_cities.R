...
cars_df <- read_xlsx(here("clean_data","private_cars_2020.xlsx"),
                       sheet = "cars_with_plates_Dec2020")
...
#renaming column
cars_df |> 
  rename("City" = "Governorate",
         "Total" = "Grand_Total") -> cars_df
...
cars_df %>% 
  dplyr::mutate(City = case_when(City %in% "Nineveh"~ "Nineveh",
                                 ...
                                 City %in% "Al-Najaf"~ "Najaf",
                                 City %in% "Al-Qadisiya"~ "Diwaniyah",
                                 City %in% "Al-Muthanna"~ "Muthanna",
                                 City %in% "Thi-Qar"~ "Thi Qar",
                                 City %in% "Maysan"~ "Maysan",
                                 City %in% "Al-Basrah"~ "Basrah",
                                 City %in% "Sulaimaniya"~ "Sulaymaniyah",
                                 City %in% "Dohouk"~ "Duhok",
                                 City %in% "Erbil"~ "Erbil")) -> cars_df
...
merge(x = map_data,y = cars_df,by.x = "ADM1_EN", by.y = "City", all.x = T) -> merged_data
...
merged_data %>% 
  mutate(nudge_y = case_when(ADM1_EN %in% c("Basrah") ~ 0.2,
                             ...
                             ADM1_EN %in% c("Babil") ~ -0.05,
                             ADM1_EN %in% c("Diyala") ~ 0,
                             ADM1_EN %in% c("Baghdad") ~ 0,
                             ADM1_EN %in% c("Thi Qar") ~ -0.1,
                             ADM1_EN %in% c("Erbil") ~ -0.1,
                             ADM1_EN %in% c("Duhok") ~ 0.07,
                             TRUE ~ 0),
         nudge_x = case_when(ADM1_EN %in% c("Diyala") ~ 0.1,
                             TRUE ~ 0),
         label_size = case_when(ADM1_EN %in% c("Baghdad") ~ 2.3,
                                ADM1_EN %in% c("Babil") ~ 2.5,
                                ADM1_EN %in% c("Sulaymaniyah") ~ 2.5,
                                ADM1_EN %in% c("Diwaniyah") ~ 2.7,
                                ADM1_EN %in% c("Kerbala") ~ 2.5,
                                TRUE ~ 3.4)) -> merged_data
...
subtitle <- ""
glue("Total Number of Cars with Plate Numbers across Iraqi Cities in 2020") -> title
...
ggplot(merged_data) + 
  ...
  color = ifelse(merged_data$ADM1_EN %in% "Baghdad",
                 "white",
                 "black"),
  lineheight = .9)+
  labs(title = title,
  ...
  guides(fill=guide_colourbar(title="Cars Count",
                              size = 14,
...
#OUTPUTTING THE FILE
ggsave(filename = "cars_number_across_cities.png",
       path = here("results"),

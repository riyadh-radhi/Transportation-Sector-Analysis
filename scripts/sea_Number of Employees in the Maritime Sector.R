...
sea_df |> 
  ...
  pivot_longer(cols = c("2017","2018","2019","2020"),
               names_to = "year",values_to = "value") -> df1

df1 |> 
  filter(str_detect(Metric, 'Number of workers')) -> df1

df1 |> 
  mutate(Metric = case_when(Metric %in% "Number of workers in State Company for Maritime Transport" ~ "State Company for Maritime Transport")) -> df1

##################
######### DF2
################

sea_df <- read_xlsx(here("clean_data","Maritime_General Company for Ports.xlsx"),
                    sheet = "general_metrics")
...
sea_df |> 
  rename("Metric" = "Details",
         ...
         "2020" = "2020.0") |> 
  pivot_longer(cols = c("2017","2018","2019","2020"),
               names_to = "year",values_to = "value") -> df2

df2 |> 
  filter(str_detect(Metric, 'Number of workers')) -> df2

df2 |> 
  mutate(Metric = case_when(Metric %in% "Number of workers in the General Company for Ports of Iraq" ~ "General Company for Ports of Iraq")) -> df2

df1 |> 
  bind_rows(df2) -> tidy_df
...
subtitle <- ""
glue("Number of Employees in the Maritime Sector") -> title
...
#OUTPUTTING THE FILE
ggsave(filename = "sea_Number of Employees in the Maritime Sector.png",
       path = here("results"),

...
tidy_df |> 
  filter(str_detect(Metric, 'Number of workers')) -> tidy_df

tidy_df |> 
  mutate(Metric = case_when(Metric %in% "Number of workers in the General Company for Ports of Iraq" ~ "Number of Workers")) -> tidy_df
...
subtitle <- ""
glue("Number of Workers of the General Company for Ports of Iraq") -> title
...
#OUTPUTTING THE FILE
ggsave(filename = "sea_number_of_workers_General Company for Ports of Iraq.png",
       path = here("results"),

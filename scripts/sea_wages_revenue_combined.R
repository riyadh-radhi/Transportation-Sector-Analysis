...
tidy_df |> 
  filter(str_detect(Metric, 'Wages') |
           str_detect(Metric, 'revenues')) -> tidy_df

tidy_df |> 
  mutate(Metric = case_when(Metric %in% "Wages and ponuses paid for the workers in State Company for Maritime Transport_billionIQD" ~ "Wages",
                            Metric %in% "Total revenues for State Company for Maritime Transport_billionIQD" ~ "Revenues")) -> tidy_df
...
subtitle <- ""
glue("State Company for Maritime Transport") -> title
...
tidy_df %>% 
  ...
  scale_x_discrete(breaks = c("2017","2018","2019","2020"),
                   expand = expansion(mult = c(0.03, 0.03))) -> plot1
...
tidy_df |> 
  mutate(Metric = case_when(Metric %in% "Wages and bonuses paid for the workers in the General Company for Ports of Iraq_billionIQD" ~ "Wages",
                            Metric %in% "Total revenues of the General Company for ports of Iraq_billionIQD" ~ "Revenues")) -> tidy_df
...
subtitle <- ""
glue("General Company for Ports of Iraq") -> title
...
tidy_df |> 
  mutate(text = case_when(year %in% 2020 ~ glue("{value} B"),
                          TRUE ~ glue("  {value} B")),
         hjust = case_when(year %in% 2020 ~ -0.05,
                          TRUE ~ 0)) -> tidy_df
tidy_df %>% 
  ggplot(aes(x=year,
             y=value,
  ...
  geom_text(aes(x=year,
                y= value + 15,
                label = glue("  {value} B")),
            size = 3,nudge_x = tidy_df$hjust,
            color= ka_col$title[[1]],
            fontface = "bold")+
  scale_x_discrete(breaks = c("2017","2018","2019","2020"),
                   expand = expansion(mult = c(0.03, 0.03))) -> plot2
...
#OUTPUTTING THE FILE
ggsave(filename = "sea_wages_revenues_combined.png",
       path = here("results"),

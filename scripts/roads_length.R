...
#renaming column
cars_df |> 
  rename("City" = "Governorate") -> cars_df
...
subtitle <- ""
glue("Total Roads Lengths across Iraqi Cities in 2020") -> title
...
ggplot(merged_data) + 
  ...
  theme_void()+
  scale_fill_gradient(low = "#f5e6e6", high = ka_col$purple[[5]])+
  ...
  guides(fill=guide_colourbar(title="Road Length in km",
                              size = 14,
...
#OUTPUTTING THE FILE
ggsave(filename = "roud_length.png",
       path = here("results"),

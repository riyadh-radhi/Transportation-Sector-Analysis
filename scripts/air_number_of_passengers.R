...
tidy_df |> 
  mutate(Metric = case_when(Metric %in% "passengers_arrivals_departuers_Iraqi airways by airport_international_internal" ~ "Iraqi Airways Passengers Number",
                            Metric %in% "passengers_arrivals_departuers_all_airways by airport_international_internal" ~ "All Airlines Passengers Number")) -> tidy_df
...
subtitle <- ""
glue("Number of Passengers in Iraqi Airports over the Years") -> title
...
#OUTPUTTING THE FILE
ggsave(filename = "air_number_of_passengers.png",
       path = here("results"),

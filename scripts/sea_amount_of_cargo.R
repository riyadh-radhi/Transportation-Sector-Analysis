...
tidy_df |> 
  filter(str_detect(Metric, 'Total cargo of')) -> tidy_df

tidy_df |> 
  mutate(Metric = case_when(Metric %in% "Total cargo of imported good ships arrive Iraqi ports_1000Ton" ~ "Imported Cargo",
                            Metric %in% "Total cargo of exported good ships departed the Iraqi ports_1000Ton" ~ "Exported Cargo")) -> tidy_df

tidy_df$value <- tidy_df$value / 1000
# KAPITA Colors -----------------------------------------------------------
...
subtitle <- ""
glue("Amount of Imported & Exported Cargo at Iraqi Ports") -> title
...
tidy_df %>% 
  ...
  geom_text(aes(x=year,
                y= value + 0.75,
                label = glue("{round(value,3)} M Ton")),
            size = 3,
...
#OUTPUTTING THE FILE
ggsave(filename = "sea_amount_of_cargo.png",
       path = here("results"),

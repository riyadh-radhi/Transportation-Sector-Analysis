...
air_df <- read_xlsx(here("clean_data","air_transportation_2020.xlsx"),
                    sheet = "flights_by_airport_domestic")
...
df_2019 |> 
  mutate(y_position = case_when(Airport %in% "Baghdad International Airport" ~ -400,
                                TRUE ~ 350),
         color = case_when(Airport %in% "Baghdad International Airport" ~ "white",
...
df_2020 |> 
  mutate(y_position = case_when(Airport %in% "Baghdad International Airport" ~ 150,
                                TRUE ~ -120),
         color = case_when(Airport %in% "Baghdad International Airport" ~ "white",
...
p2
glue("Number of Domestic Flights of the Iraqi Airways by Airport")  -> title
...
ggsave(filename = glue("air_number_of_flights_by_airport_domestic.png"),
       path = here("results"),

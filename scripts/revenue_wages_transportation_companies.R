...
land_df |> 
  select(Year, contains("Revenue"),contains("Wages")) |> 
  pivot_longer(cols = c(contains("Revenue"),contains("Wages")),
...
land_transport_df |> 
  mutate(Metric = case_when(Metric %in% "Revenue (The General Company for Land Transport)" ~ "Revenue",
                            Metric %in% "Wages (The General Company for Land Transport)" ~ "Wages")) -> land_transport_df

land_transport_df$Value_in_billion <- (land_transport_df$Value * 1000000) / 1000000000
...
travellers_df |> 
  mutate(Metric = case_when(Metric %in% "Revenue (The General Company for Travellers and Delegates)" ~ "Revenue",
                            Metric %in% "Wages (The General Company for Travellers and Delegates) " ~ "Wages")) -> travellers_df


travellers_df$Value_in_billion <- (travellers_df$Value * 1000000) / 1000000000
...
subtitle <- ""
glue("Revenues & Wages of the General Company for Travellers and Delegates") -> title
...
subtitle <- ""
glue("Revenues & Wages of the General Company for Land Transport") -> title
# 
# caption <- "<span style = 'color: #194354'>Source:</span>
...
#OUTPUTTING THE FILE
ggsave(filename = "revenues_wages.png",
       path = here("results"),

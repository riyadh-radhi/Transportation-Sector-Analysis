...
tidy_df |> 
  filter(str_detect(Metric, 'Number of ships')) -> tidy_df

tidy_df |> 
  mutate(Metric = case_when(Metric %in% "Number of ships arrive Iraqi ports (goods transportation)" ~ "Arrived Ships",
                            Metric %in% "Number of ships departed the Iraqi ports (goods transport)" ~ "Departed Ships")) -> tidy_df
...
subtitle <- ""
glue("Number of Ships Used for Goods Transportation in the Iraqi Ports") -> title
...
#OUTPUTTING THE FILE
ggsave(filename = "sea_number_of_ships_good_transportations.png",
       path = here("results"),

...
train_df |> 
  rename( "Passengers Transport Revenues" = "Revenues (million ID) from Transport Passengers",
          "Goods Transport Revenues" = "Revenues (million ID) from Transport Revenues (million ID) from Transport Passengers") |> 
  select(Year,`Passengers Transport Revenues`,`Goods Transport Revenues`) |> 
  pivot_longer(cols = c(`Passengers Transport Revenues`,`Goods Transport Revenues`),
               names_to = "Metric", values_to = "Value") -> train_df 
...
subtitle <- ""
glue("Revenues from Passengers and Goods Transported via the Railways") -> title
...
y_title <- "Revenues in Million IQD"
x_title <- ""
...
train_df |> 
  mutate(y_text_margin = case_when(Year %in% 2018 & Metric %in% "Passengers Transport Revenues" ~ -200,
                                   TRUE ~ 200)) -> train_df
...
train_df %>% 
  ...
  geom_text(aes(x=Year,
                y= Value + y_text_margin,
                label = glue("{comma(Value)} M")),
            size = 3.5,
...
#OUTPUTTING THE FILE
ggsave(filename = "train_revenue_passengers_goods_years.png",
       path = here("results"),

...
tidy_df |> 
  filter(str_detect(Metric, 'Gross')) -> tidy_df

tidy_df |> 
  mutate(Metric = case_when(Metric %in% "Gross cargo of the imported and expoted goods ships by owned ships_1000ton" ~ "Owned Ships Cargo",
                            Metric %in% "Gross tonnage of exported and imported cargo vessels by chartered vessels_1000ton\n" ~ "Chartered Ships Cargo")) -> tidy_df
...
subtitle <- ""
glue("Amount of Imported & Exported Cargo by the State Company for Maritime Transport") -> title
...
y_title <- "Cargo in Thousands Tons"
x_title <- ""
...
#OUTPUTTING THE FILE
ggsave(filename = "sea_amount_of_cargo_State Company for Maritime Transport.png",
       path = here("results"),

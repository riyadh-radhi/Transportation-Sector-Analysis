...
train_df |> 
  ...
  pivot_longer(cols = c("2016","2017","2018","2019","2020"),
               names_to = "Year", values_to = "Value") -> train_df 
...
subtitle <- ""
glue("Number of Working Train Cars across the Years") -> title
...
y_title <- "Train Cars Count"
x_title <- ""

########### data preprocessing 
library(forcats)
train_df |> 
  filter(!Year %in% "2016") -> train_df

train_df |> 
  mutate(Metric = factor(Metric),
         Metric = fct_reorder(Metric,Value, .desc = TRUE)) -> train_df
...
#OUTPUTTING THE FILE
ggsave(filename = "train_Number of Working Train Cars across the Years.png",
       path = here("results"),
